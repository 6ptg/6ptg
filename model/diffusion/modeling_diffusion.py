...
from model.diffusion.vqgan_vae import NullVQGanVAE, VQGanVAE
import torch
import math
...
class Unet(nn.Module):
    ...
    def forward_with_cond_scale(
        ...
        **kwargs
        ...
        null_logits = self.forward(*args, image_cond_drop_prob = 1., **kwargs)
...
class DiffusionPrior(nn.Module):
    ...
    def sample(
        ...
        text_embed = repeat(text_embed, 'b ... -> (b r) ...', r=num_samples_per_batch)
        ...
        top_sim_indices = text_image_sims.topk(k=topk).indices

        # top_sim_indices = repeat(top_sim_indices, 'b 1 -> b 1 d', d = image_embed_dim)
        top_sim_indices = repeat(top_sim_indices, 'b k -> b k d', d=image_embed_dim, k=topk)

        top_image_embeds = image_embeds.gather(1, top_sim_indices)
        # return rearrange(top_image_embeds, 'b 1 d -> b d')
        return rearrange(top_image_embeds, '1 b d -> b d')
...
class Decoder(nn.Module):
    def __init__(
        ...
        if exists(image_size) or exists(image_sizes):
            ...
            raise Exception('either image_size, image_sizes, or clip must be given to diffusion')
        ...
        self.can_classifier_guidance = image_cond_drop_prob > 0.
    ...
    def p_mean_variance(self, unet, x, t, image_embed, noise_scheduler, lowres_cond_img=None,
        ...
        assert not (
                    cond_scale != 1. and not self.can_classifier_guidance), 'the diffusion was not trained with conditional dropout, and thus one cannot use classifier free guidance (cond_scale anything other than 1)'
    ...
    @torch.no_grad()
    def p_sample(self, unet, x, t, image_embed, noise_scheduler, cond_scale=1.,
    ...
    def p_losses(self, unet, x_start, times, *, image_embed, noise_scheduler, lowres_cond_img=None,
        ...
        # at the first timestep return the diffusion NLL, otherwise return KL(q(x_{t-1}|x_t,x_0) || p(x_{t-1}|x_t))
    ...
    def sample(
        ...
        assert self.unconditional or exists(
            image_embed), 'image embed must be present on sampling from diffusion unless if trained unconditionally'
    ...
    def forward(
        ...
        if not exists(image_embed) and not self.unconditional:
            assert exists(
                self.clip), 'if you want to derive CLIP image embeddings automatically, you must supply `clip` to the diffusion on init'
            image_embed, _ = self.clip.embed_image(image)
        ...
        losses = self.p_losses(unet, image, times, image_embed=image_embed,
